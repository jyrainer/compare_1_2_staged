...
def classify_transforms(size=224, mean=(0.0, 0.0, 0.0), std=(1.0, 1.0, 1.0)):  # IMAGENET_MEAN, IMAGENET_STD
    ...
    if any(mean) or any(std):
        ...
        return T.Compose([ClassifyLetterBox(size), ToTensor(), T.Normalize(mean, std, inplace=True)])
    else:
        # return T.Compose([CenterCrop(size), ToTensor()])
        return T.Compose([ClassifyLetterBox(size), ToTensor()])
...
def classify_albumentations(
    ...
    try:
        ...
        if augment:  # Resize and crop
            ...
            T.append(A.geometric.resize.LongestMaxSize(max_size=224, interpolation=1, always_apply=False, p=1))
            T.append(A.PadIfNeeded(min_height=224, min_width=224, border_mode=0, value=(114,114,114)))
            ...
            # T = []
            # T = [A.SmallestMaxSize(max_size=size), ClassifyLetterBox(size)]
